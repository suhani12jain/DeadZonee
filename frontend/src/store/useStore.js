...
const useStore = create((set, get) => ({
  ...
  // ── What-If Agent (preview only; not persisted) ────────────────────────────
  whatIf:            null,   // last /api/what-if response payload (minus status)
  whatIfViewMode:    'delta', // 'delta' | 'new_dead' | 'recovered'
  whatIfLoading:     false,
  whatIfManualCells: [],     // { row, col, zone_type } queued for next run
  setWhatIf:         (w) => set({ whatIf: w }),
  clearWhatIf:       () => set((s) => ({
    whatIf: null,
    whatIfViewMode: 'delta',
    whatIfManualCells: [],
    overlays: { ...s.overlays, whatif: false },
  })),
  setWhatIfViewMode: (m) => set({ whatIfViewMode: m }),
  setWhatIfLoading:  (v) => set({ whatIfLoading: v }),
  addWhatIfManualCell: (row, col, zone_type) =>
    set((s) => {
      const key = `${row}_${col}`
      const rest = s.whatIfManualCells.filter((c) => `${c.row}_${c.col}` !== key)
      return { whatIfManualCells: [...rest, { row, col, zone_type }] }
    }),
  removeWhatIfManualCell: (row, col) =>
    set((s) => ({
      whatIfManualCells: s.whatIfManualCells.filter(
        (c) => !(c.row === row && c.col === col),
      ),
    })),
  clearWhatIfManualCells: () => set({ whatIfManualCells: [] }),
  ...
  reset: () => set({
    ...
    whatIf: null, whatIfViewMode: 'delta', whatIfLoading: false, whatIfManualCells: [],
    overlays: { heatmap: true, deadzone: false, interference: false, priority: false, zones: true, whatif: false },
